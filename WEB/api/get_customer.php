...
if (!empty($data["id_business"])) {
    ...
    if ($resuser->num_rows > 0) {
        while ($row = $resuser->fetch_assoc()) {
            ...
            $stmt_cskh = $conn->prepare("SELECT id_user 
                            FROM cskh
                            WHERE id_customer = ?");
            $stmt_cskh->bind_param("s", $row['id_customer']);
            $stmt_cskh->execute();
            $res = $stmt_cskh->get_result();
            $row['cskh'] = $res->fetch_assoc();
    ...
    if (!empty($data["id_user"])) {
        ...
        if (!empty($data["id_customer"])) {
            ...
            $customer = [];
            $stmt = $conn->prepare("SELECT * FROM customerinfo WHERE id_customer = ?");
            ...
            if ($res->num_rows > 0) {
                while ($row = $res->fetch_assoc()) {
                    ...
                    if (!empty($row['more'])) {
                        ...
                        // Lấy danh sách trường từ bảng custominfo theo id_business
                        $stmt_custom = $conn->prepare("SELECT name FROM custominfo WHERE id_business = ? ORDER BY id_custominfo ASC");
                        $stmt_custom->bind_param("s", $row["id_business"]);
                        $stmt_custom->execute();
                        $res_custom = $stmt_custom->get_result();
